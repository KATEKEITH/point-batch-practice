package com.practice.multicore.lock;
...
public class UserLevelLockFinal {
    ...
    public <T> T executeWithLock(String userLockName,
        ...
        try (Connection connection = dataSource.getConnection()) {
            try {
                ...
                log.info("success getLock=[], timeoutSeconds ], connection=[]", userLockName, timeoutSeconds,
                        connection);
                return supplier.get();
    ...
    private void checkResultSet(String userLockName,
        ...
        try (ResultSet resultSet = preparedStatement.executeQuery()) {
            if (!resultSet.next()) {
                log.error("USER LEVEL LOCK 쿼리 결과 값이 없습니다. type = [], userLockName ], connection=[]", type, userLockName,
                        preparedStatement.getConnection());
                throw new RuntimeException(EXCEPTION_MESSAGE);
            }
            int result = resultSet.getInt(1);
            if (result != 1) {
                log.error("USER LEVEL LOCK 쿼리 결과 값이 1이 아닙니다. type = [], result ] userLockName ], connection=[]", type,
                        result, userLockName, preparedStatement.getConnection());
                throw new RuntimeException(EXCEPTION_MESSAGE);
            }
        }
    }
